...
stateDiagram-v2
  ...
  STATION: 工位停靠
  CARRY: 搬运中 · 整带保持停止
  SORTED: 框内稳定 0.2 s · 完成位锁存
  MISSED: 掉地/放错区域 · 整带保持停止

  INFEED --> FLOW: 每个控制步覆写水平速度
  FLOW --> FLOW: 高度窗口 + 带面范围判据通过
  FLOW --> STATION: 到达工位
  STATION --> CARRY: 被机器人拎离带面
  CARRY --> SORTED: 箱底入接收区且低速连续驻留
  CARRY --> MISSED: 掉地或放到目标框外
  SORTED --> FLOW: 放行下一格
...
- **判据只用几何**（高度窗口 + 带面范围），所以物体被拎起来的瞬间就自动脱离驱动，
  不会出现「抓在手里还被传送带拖走」。
- **离带不等于放行**：每个纸箱必须仍在带上，或已在两台机器人身后的任一蓝色分拣框
  内低速稳定约 0.2 s。搬运中、弹跳、掉地或放错区域都会继续保持停线；完成位会锁存到
  F12/DDS/env reset，框内堆叠、碰撞或清框不会让已完成周期反悔。
