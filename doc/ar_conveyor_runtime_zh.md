...
flowchart LR
  ...
  KIT --- PROV
  PROV --- BRIDGE
  BRIDGE -- "机器人状态 / 灵巧手状态" --> DEPLOY
  DEPLOY -- "关节指令 / 灵巧手指令" --> BRIDGE
  KIT <-- "视角锚定 · 手柄按键" --> RT
  RT --- HMD
```
...
| 约定 | 内容 |
|---|---|
| 机器人本体 | 29 个本体关节 + 14 个灵巧手关节 = 43 DoF；URDF 在启动时由**与策略训练同源的机器人描述资产**在本地合成（见下方说明） |
...
flowchart TB
  ...
  ENV --> ARGS["参数解析"]
  ARGS --> SW1{"SONIC 任务?"}
  SW1 -- 是 --> D1["动作源改写为 DDS 锁步 · 控制频率 50 Hz · 强制灵巧手 DDS · 倒地自动复位开"]
  D1 --> SW2{"选了手柄设备?"}
...
flowchart LR
  ...
  OBS --> IN --> PUB
  PUB -- "机器人状态 · 副 IMU · 灵巧手状态 · 仿真状态" --> WIRE(("总线"))
  WIRE -- "关节指令 · 灵巧手指令 · 复位命令" --> SUB
  SUB --> OUT --> GETA
```
...
三个触发源汇到同一个入口，因此三条路径的行为完全一致——不存在「手动复位和自动复位不一样」的问题。

```mermaid
flowchart TB
  T1["手柄按键 · （默认左手 X）"] --> GATE["复位请求"]
  T2["DDS 复位命令"] --> GATE
  T3["倒地自动检测 · 倾角 / 根部高度 + 去抖 + 冷却"] --> GATE
...
**手柄按键那条路有个约束**：按键回调跑在渲染引擎的消息泵调用栈里——也就是渲染进行到一半的时候。
在那儿就地复位等于在画面渲染途中改物理场景。所以回调**只置一个请求位**，
真正的复位交给主循环，在处理 DDS 复位命令的同一处执行。默认 2 秒去抖，
防止手抖连按把复位纪元反复推给控制器。
...
stateDiagram-v2
  ...
  FLOW --> STATION: 到达工位
  FLOW --> OFFBELT: 被机器人拎起或掉出带面
  STATION --> INFEED: 整场景复位
  OFFBELT --> INFEED: 整场景复位
```
