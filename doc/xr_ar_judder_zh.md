...
> **若只要存在任何一个可见 overlay 就能逼它转入合成模式，则不必牺牲立体、
> 也不必改 Isaac 一行代码。**

⚠️ 但这条路**不是免费的**：实测 dashboard 打开时应用侧帧时间 +5–6ms、主循环
~49Hz → 36–40Hz（4/4 命中，见 §2.1 的独立发现）。原先写的"代价极小"已按实测更正——
**不改代码，但要付帧率**。conveyor 本就贴着预算跑，这条路在它上面更紧。
...
**基线（主场景 + GR00T 在线，暖机 1 小时，20 分钟 / 235 个统计窗口）**：

| 指标 | 全段 | 最好的连续 3 分钟 |
|---|---|---|
| 主循环 | 31.3–50.3 Hz（均值 43.7±5.9） | 47.6–50.3 Hz |
| 渲染成本 | 8.0–14.6 ms/帧 | 8.0–9.0 ms/帧 |
| A（等 ack） | 0.7–1.9 ms（全段稳定） | 同 |

对照非 AR 记录的 A≈0.9 / E≈6.2 / R≈6.4：只开 `--xr` 就让 E、R 各涨约 2–3ms，
S 已经归零（预算穿底）。⚠️ 但这个对照**不干净**：非 AR 走精简 experience
`apps/isaaclab.sonic.kit`（少 71 个扩展），XR 会自动跳过精简版改用完整的
`isaaclab.python.xr.openxr.kit`，所以这 2–3ms 里混了 kit CPU 地板的差，不能全记到
XR 头上。要拆清得在同一 experience 下对比，本节的归因不依赖这个数。

⚠️ **基线不是平的，别只取一个数**：全段摆动 31–50Hz。A 全程稳定 ⇒ 与外部锁步无关；
E 摆 8.5–14.2ms、**R 摆 8.0–14.6ms**，摆动主要在 R。原因见下，它同时是一条独立发现。

#### ⭐ 独立发现：dashboard 一开，应用帧时间 +5–6ms（4/4 命中，2026-07-30）

把 R>12ms 的区间与 `vrcompositor.txt` 的 dashboard/控制器交互事件对齐，**四段全中**，
每段都在事件后 4–7 秒开始（延迟由 5s 统计窗口 + 100 样本滑动平均解释）：

| dashboard 事件 | R>12ms 区间 |
|---|---|
| 10:13:48 | 10:13:52 → 10:17:28 |
| 10:24:01 | 10:24:08 → 10:26:14 |
| 10:26:48 | 10:26:54 → 10:27:59 |
| 10:29:03 / 10:29:23 | 10:29:09 → 10:29:34 |

**dashboard 打开时，我们的 CPU 工作量毫无变化，但 `sim.render()` 的墙钟从 8–9ms
涨到 13–14.6ms，主循环从 ~49Hz 掉到 36–40Hz。** 机制：compositor 退出直通、改为自己
合成后，帧节奏变了，`xrWaitFrame` 在渲染调用里等得更久。

这条有三层后果：

1. **给 §4.1 的 overlay 路线定了价。** "逼 compositor 退出直通"不是免费的：
   代价约 **5–6ms/帧的应用墙钟**，主场景 ~49Hz → ~37Hz。conveyor 本来就贴着 20ms
   预算跑（E≈10、R≈7、S≈0），这条路在 conveyor 上只会更紧。之前记的"代价极小——
   不牺牲立体、不改 Isaac 一行代码"要补上这一项：**不改代码，但要付帧率**。
2. **可能解释掉那条历史迷案。** §1 记的 2026-07-28"匀速 36fps 就流畅"——
   dashboard 开着时应用**正好掉到 36–40Hz**。也就是说那个"36fps"很可能不是巧合选中的
   工况，而是**dashboard 本身造成的**；于是那次观测里"36fps"和"流畅"都是 dashboard
   的产物。（本条是有数字支撑的假设，未单独复现。）
3. **R 不能用作归因指标。** `R` = 我们的渲染 CPU + `xrWaitFrame` 阻塞，两者混在一格里，
   正好把本节要拆的嫌疑 A 与 B **重新搅在一起**。因此归因只认两个口径：探针自报的
   **回调内部耗时**（纯 CPU）与**主循环 Hz**（端到端效果），且各档必须在
   compositor 状态一致的窗口里测。测量脚本已加三项干扰检测：别的 VR 应用连接、
   dashboard/控制器交互、同时存在的第二个 sim。
