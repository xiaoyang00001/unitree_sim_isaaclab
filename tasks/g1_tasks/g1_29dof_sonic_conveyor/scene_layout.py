...
# 默认队首永远保持 D01 + 压皱 D02；0.60 m 窄带的后 15 件只从横向不超过
# 0.4526 m 的八种小箱/软包中做带 seed 的平衡随机。C01/C02 的横向尺寸均为
# 0.50 m，窄带两侧各只剩 50 mm，因此保留注册供显式 PATTERN 调试，但不再进入
# 生产默认池。每轮先让池中每种资产各出现一次，再进入下一轮，避免纯抽样漏掉
# 某种外观。排序只用 SHA-256，不依赖 Python hash / 全局 random / 系统熵，所以
# Linux 权威端与 Windows viewer 在相同 seed 下会得到逐槽一致的 USD。
...
def resolve_belt_box_positions(
    ...
    *,
    ...
    固定为 D01/D02，其余 15 件从八种窄带安全的小箱/软包中做带 seed 的平衡随机；
    显式 ``ISAACLAB_BELT_BOX_PATTERN`` 仍可从全部十种已注册箱型中完整覆盖，包括
    C01/C02。出生间距 ``spawn_pitch`` 只决定初始队形；停下来后的实际队距由
    **每个箱子自己的半长**加净间隙 ``queue_gap`` 决定，因此显式混入 C 型大箱时
    也仍使用各自真实占位。
...
def resolve_scene_layout(environ: Mapping[str, str]) -> ConveyorSceneLayout:
    ...
    if totes_on_conveyor:
        ...
        # 停止线随双机作业组一起下移，保持队首/第二件箱分别与 robot_1/2 对齐。
