...
def queue_drive_mask(
    ...
    *,
    ...
    而出生间距（默认 0.75）大于任何一对箱子的最小净距。它只在两箱因摩擦/质量差异
    缓慢漂移到快要追尾时才介入，约束是

        ys[i] > (前车 y + 前车半长) + 自己半长 + queue_gap

    即**前车尾部 + 自己半长 + 净间隙**，而不是一个统一的中心距——默认池的八种
    小箱/包尺寸不同，显式 PATTERN 还可加入 C 型大箱；统一中心距要么让大箱互相
    穿模、要么在小纸箱/软包之间留出突兀的空档。按各自半长算，无论怎么交错，
    兜底的净空隙都恒为 ``queue_gap``。
