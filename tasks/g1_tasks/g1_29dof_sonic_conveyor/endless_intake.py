...
* 弯道西口（公头）插进 **ConveyorBelt_A05** 2 m 短直段的母口，共**五段** A05
  ...
  x 连排到 -29.58）**北侧擦过去**（Δ=0.25 整体北移换来的通道；原宽基线净距
...
遮挡核算（Δ=0.25，窄化前历史 mesh 级射线；眼位
E1=(-4.54,14.698,1.6)/E2=(-6.7,14.698,1.6)，箱顶最高 c01=1.0223）：

当前 robot_1/2 随居中箱位向车道内移、又随双机作业组沿世界 -Y 下移 0.40 m 后，
近似眼位分别为 E1=(-4.74,14.298,1.6)/E2=(-6.50,14.298,1.6)。以下历史结论需要
用新眼位重跑，暂只作为遮挡结构与回生点选择的保守参考。
...
NARROW_WIDTH_SCALE = NARROW_LANE_WIDTH / SOURCE_LANE_WIDTH
...
XLEG_COUNT = 5
XLEG_YAW_DEG = -90.0
# A05 的横向是资产局部 X；只缩 X，局部 Y 的 2 m 输送长度与段间插接不变。
# 资产车道中线在局部 x=0.5014 m。缩放后同步下调世界 translate.y，确保
# yaw=-90° 下世界车道中线仍精确落在 CURVE_WEST_LANE_Y。
XLEG_SCALE = (
    CONVEYOR_UNIT_SCALE * NARROW_WIDTH_SCALE,
    CONVEYOR_UNIT_SCALE,
    CONVEYOR_UNIT_SCALE,
)
XLEG_SOURCE_LANE_CENTER_X = 0.5014
XLEG_POS_Y = CURVE_WEST_LANE_Y + XLEG_SOURCE_LANE_CENTER_X * NARROW_WIDTH_SCALE
XLEG_POSITIONS = (
    (-7.0979, XLEG_POS_Y, 0.003),
    (-9.0830, XLEG_POS_Y, 0.003),
    (-11.0681, XLEG_POS_Y, 0.003),
    (-13.0532, XLEG_POS_Y, 0.003),
    (-15.0383, XLEG_POS_Y, 0.003),
)
# A05 源资产局部 X AABB（按 0.01 基准换算为米）；窄化后世界 Y AABB 由
# yaw=-90° 的 y_world=t_y-x_local*scale 推出。带宽缩到 0.60 m 后，机身南缘
# 对排 B 端护板北缘的净距从 83.3 mm 增至约 275 mm。
XLEG_SOURCE_LOCAL_X_RANGE = (-0.074051513671875, 1.076900634765625)
XLEG_VISUAL_Y_RANGE = (
    XLEG_POS_Y - XLEG_SOURCE_LOCAL_X_RANGE[1] * NARROW_WIDTH_SCALE,
    XLEG_POS_Y - XLEG_SOURCE_LOCAL_X_RANGE[0] * NARROW_WIDTH_SCALE,
)
XLEG_AABBS = (
    ((-9.0979, -7.0978), XLEG_VISUAL_Y_RANGE, (0.003, 1.1693)),
    ((-11.0830, -9.0829), XLEG_VISUAL_Y_RANGE, (0.003, 1.1693)),
    ((-13.0681, -11.0680), XLEG_VISUAL_Y_RANGE, (0.003, 1.1693)),
    ((-15.0532, -13.0531), XLEG_VISUAL_Y_RANGE, (0.003, 1.1693)),
    ((-17.0383, -15.0382), XLEG_VISUAL_Y_RANGE, (0.003, 1.1693)),
)
...
#   窄支线机身南缘约 19.6697 从其北侧通过（对护板/架板/钢架的净距约
#   275/324/362 mm；原宽基线分别为 83.3/132.2/170.6 mm）。
...
LANE_HALF_WIDTH = 0.30       # = conveyor_drive.BELT_WIDTH / 2（交叉断言）
...
# * 拐角补块：覆盖弧段窄车道摆幅（车道 ±0.30 ⇒ 半径 [1.10,1.70]）的包围矩形，
#   南缘接主线碰撞板末端 18.47，整块落在弯道足迹内；
# * 支线条带：车道中线 ±0.30，东接拐角补块、西到段 5 滚筒可用端（西端延长
#   3970.2 mm，段间 ≈41 mm 滚筒断口照旧一并补齐）。
# 板厚/顶面与主线一致（顶 z=0.772，厚 0.04）。
# ⚠️ 条带南缘 19.7534 与排 B 端护板北缘 19.3946 现在 x 区间**有重叠**（条带西伸
#   到 -17.034，护板在 -13.734..-13.386），但 y 净距 358.8 mm 充裕——旧版
...
CORNER_PLATE_X_RANGE = (CORNER_CENTER[0], MAIN_LANE_X + LANE_HALF_WIDTH)   # (-7.02, -5.32)
CORNER_PLATE_Y_RANGE = (18.47, BRANCH_LANE_Y + LANE_HALF_WIDTH)            # (18.47, 20.3534)
