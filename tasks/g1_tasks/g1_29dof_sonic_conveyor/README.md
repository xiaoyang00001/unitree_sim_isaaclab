...
箱/包的质心都放在路径中心线上，不再给前两箱添加朝机器人一侧的横向偏移。最宽的
`c01/c02` 横向尺寸为 `0.50 m`，在窄带上两侧各余 `50 mm`。robot_1/2 同步向车道内侧
平移 `0.20 m`，保持机器人到各自箱心原有的 `0.88 m` 横向距离，双手可从箱体两侧抱住
中部，而不是追随偏心箱位。
...
- **回生点遮挡：既有高度下 E2 达成全遮，C02 待复核；E1 几何上无全遮解（缝隙残余）**。Δ=0.25 北移 +
  ...
  结论只作保守参考。历史核算最高只覆盖到 C01；当前随机池的 C02 箱顶约 `z=1.086`，
  比 C01 再高 61 mm：
