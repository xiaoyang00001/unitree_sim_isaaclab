...
| 值 | 布局 | 关键世界坐标 |
|---|---|---|
| `1`（默认） | 17 个箱/包沿**西拐入口弯道路径**以 pitch 0.75 排在工位上游、挡停放行流向工位；现有两台机器人保持原工位，另有 3 台纯显示 G1 在支线队尾、主线和支线中段分散站位，不组成面对面队列 | 现有工位 `y=14.398`（机器人 `x=-4.54 / -6.7` 不变）；新增站位 `(-12.70,18.9534)` 位于最后一个物体南侧偏东 1 m 并朝西北正对它、`(-6.70,17.20)` 朝 `+X`、`(-9.50,21.15)` 朝 `-Y`；出生槽位 s=`11.06 − 0.75k (k=0..16)`；`y_stop=14.398` |
