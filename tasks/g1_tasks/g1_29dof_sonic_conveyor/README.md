...
流水线两端原本是断头，料筐在带面上凭空出现/消失。`ISAACLAB_CONVEYOR_SHROUD` 在生效
端口的外侧贴一台**纯视觉**的隧道式 X 光安检机（DigitalTwin
`SecurityBaggageScanner_A01_01`，全资产包里唯一自带隧道洞口 + 铅胶条帘的件），
让带面看起来是从设备里穿出来 / 钻进去的：
...
拼写错误直接报错。`ISAACLAB_CONVEYOR_SHROUD_FACE_Y=<float>` 可覆盖“迎风面”
（罩子朝流水线那一侧的端面）的世界 y，整体沿 Y 平移罩子。

| 布局 | 端口 | 迎风面 y | 罩子占位 y | 世界坐标 pos / yaw |
|---|---|---|---|---|
| `ISAACLAB_TOTES_ON_CONVEYOR=1`（默认） | 入料端（带体到 18.222） | `18.300` | `[18.300, 20.848]` | `(-5.620, 19.57424, -0.07935)` / `0°` |
| `ISAACLAB_TOTES_ON_CONVEYOR=0` | 末尾出料端（带尾 10.188） | `9.660` | `[7.112, 9.660]` | `(-5.620, 8.38576, -0.07935)` / `180°` |

两种布局共用同一台机身（0.843 × 2.548 × 1.502 m），x 与流水线中线 `-5.62` 对齐，
整体下沉 `0.0794` 让机身自带滚筒面与带面 `z=0.772` 同高，机脚/脚轮随之埋进地面、
车架四角正好落地。条帘洞口 `z[0.777, 1.261]`、净宽 `0.632`，半尺寸料筐（0.3 × 0.2 ×
0.15 m）通过绰绰有余。转 180° 是为了让机身同一个端面在两种布局下都朝着流水线。
...
用 `UsdFileCfg.scale=0.01` 补换算因子。

⚠️ **出料端为什么离带尾 0.53 m**：背景里 `blue_sorting_bin_03` 蓝料箱堆就压在带尾外侧
`x[-6.221,-4.555] y[8.757,10.092] z[0.003,0.420]`（距带尾仅 0.096 m），而安检机的隧道
机柜是**落地**的（`z` 从 0.078 一直到 1.498，占满机身中段 `y ±0.350`），贴着带尾放会
让机柜整块插进那堆料箱。迎风面因此退到 `9.66`，机柜与料箱堆留 0.021 m 余量；这段空隙
视觉上正好被那堆料箱填住。残留穿模只剩车架近端护板的一条 `0.59 × 0.034 × 0.034 m`
边角（`z[0.061,0.095]`）埋在料箱堆里，贴地且在机身阴影下。入料端外侧则实测完全干净
（`x[-7.6,-3.6] × y[18.25,21.5] × z<3.5` 内 0 个 Gprim）。
...
- **端口遮挡罩当前只贴在端口外侧提供视觉观感，未真正遮住料筐出生瞬间**
  （tote2 出生于 `y=18.0`，后缘距入料端仅 `0.122 m`）。要真正遮住需二选一：
  ①罩子做出进深覆盖 `y∈[17.9,18.5]`；②延长流水线并后移 `TOTE_SPAWN_Y_*`，
  后者会牵动复位落点与行程时间。
  走①还要一并解决横向尺寸：安检机条帘净开口只有 `0.632 m` 宽，而两条筐道
  在 `x=-5.35 / -5.89`、筐宽 `0.30`，横向总跨度 `0.69 m` 已经超过洞口，
  需要收窄筐道或放大机身（放大又会加深下沉，见上面的取舍说明）。
- 遮挡罩只做过离线 pxr 静态占位/穿模核对，尚未在 Kit 里目视验收；出料端与
  `blue_sorting_bin_03` 的边角残留穿模也只按包围盒+点集判定，未看过实际渲染。
- `ISAACLAB_CONVEYOR_PROPS=legacy_props` 下入料端会多出 `pushcart_2`
  （`y[18.570,20.218]`），与入料端罩子占位 `y[18.300,20.848]` 重叠；默认的
  `layout` 道具模式在 `TOTES_ON_CONVEYOR=1` 时根本不生成 `pushcart_2`，所以只有
  做 legacy A/B 时才需要用 `ISAACLAB_CONVEYOR_SHROUD_FACE_Y` 或 `=off` 避让。
...
| props/infeed_scanner_visual.usda | 本仓库任务专用视觉薄层 | 端口遮挡罩；以 S3 绝对 URL 引用 DigitalTwin `SecurityBaggageScanner_A01_01`（cm 原生，任务侧补 0.01 换算），本身不 author 任何物理 |
