...
| 值 | 布局 | 关键世界坐标 |
|---|---|---|
| `1`（默认） | 17 个箱/包沿**西拐入口弯道路径**以 pitch 0.75 排在工位上游；第二机器人及其西侧工作台、分拣箱沿 `+Y` 错开 `0.75 m`，队首两箱成组到位后供两机器人同时抱取，流水线持续停住且不再补位；三机 host/viewer 下 Robot3/PeerRobot3 接管近位 `StandbyRobot2`，另有 2 台纯显示 G1 | 第一/第二机器人工位分别为 `y=14.398 / 15.148`（`x=-4.54 / -6.7`）；Robot3 站位 `(-6.70,17.95)` 朝 `+X`，纯显示站位 `(-12.70,18.9534)` 朝西北、`(-9.50,21.15)` 朝 `-Y`；出生槽位 s=`11.06 − 0.75k (k=0..16)`；`y_stop=14.398` |
...
`scene_state` 同步。旧双机模式生成三台；三机 host/viewer 的近位 `StandbyRobot2` 由 Robot3/PeerRobot3
