...
`0.30 m`，机器人 Y 变为 `14.098/14.848`，收纳箱中心 Y 变为约
`15.248/15.998`；默认停止线同步下移到 `14.098`，所以前两箱仍分别正对两台机器人。
...
（s 参数化对 Δ 整体平移**不变**；本轮作业组另沿世界 `-Y` 下移 `0.30 m`，所以工位
s 相应增加 `0.30`）：工位 `y=14.098 ⇒ s=12.4945`、回收线
...
排在工位 `s_stop=12.4945`（`y_stop=14.098`）上游，以队首 `s=11.06` 为锚、pitch 0.75
...
直排，**默认 5 箱**（上游带面约 4.37 m，仍装不下 17），出生 y
...
`s 11.06→12.4945 ≈ 1.435 m`（按既有实测带速约 0.244 m/s 粗估约 5.9 s 流到
工位）。直线回退形态的上游带面约为 `18.47 - 14.098 ≈ 4.37 m`，
`(count-1)×pitch + 端部半长 + 队首行程 ≤ 4.37`，超了启动即报"队尾悬出带面"。
...
| `ISAACLAB_CONVEYOR_SPEED` / `_Y_STOP` / `_ENABLED` | `0.3` / `14.098` / `1` | 速度、工位、总开关；默认停止线跟随 robot_1 下移，弯道形态要求 y_stop 在主线段（<18.6534） |
...
`--place-lead-after` 仍兼容；弯道形态队首行程约 1.435 m，默认 1600 步还为
整列稳定和离线后持续停线留出时间）：
...
- 停位有 0.06~10 mm 的散布（越线后驱动关闭、靠摩擦停住，各箱摩擦略有差异），smoke 容差
  ...
  出生 0.75），但**不要指望它精确恒定**。
- **直线回退形态**的队首行程随停止线下移增至 1.10 m（按 0.244 m/s 粗估约
  4.5 s）。带面上游总长约 4.37 m，间距、行程、数量三者仍需一起权衡。
...
- **回生点遮挡：既有高度下 E2 达成全遮，C02 待复核；E1 几何上无全遮解（缝隙残余）**。Δ=0.25 北移 +
  ...
  眼位再随作业组下移，约为 E1=`(-4.74,14.398,1.6)` /
  E2=`(-6.50,14.398,1.6)`，需另行重跑，因此这些
