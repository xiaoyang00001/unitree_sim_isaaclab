...
| 值 | 布局 | 关键世界坐标 |
|---|---|---|
| `1`（默认） | 17 个箱/包沿**西拐入口弯道路径**以 pitch 0.75 **铺满**工位上游、挡停放行流向工位；现有两台机器人保持原工位，另有 3 台纯显示 G1 在支线队尾、主线和支线中段分散站位，不组成面对面队列 | 现有工位 `y=14.398`（机器人 `x=-4.54 / -6.7` 不变）；新增站位 `(-12.70,18.9534)` 位于最后一个物体南侧偏东 1 m 并朝西北正对它、`(-6.70,17.20)` 朝 `+X`、`(-9.50,21.15)` 朝 `-Y`；出生槽位 s=`8.06 − 0.75k (k=0..16)`；`y_stop=14.398` |
...
物理托面：两块 kinematic 补板与主线碰撞板同 z 同摩擦——拐角补块
`x[-7.02,-5.17] y[18.47,20.5034]` + 支线条带 `x[-17.034,-7.02] y=20.0534±0.45`
（西端随支线延长 3.97 m，顺带补齐段间 41 mm 滚筒断口；条带南缘 `19.6034` 与排 B
端护板北缘 x 已重叠但 y 净距 208.8 mm——旧版 7 mm 错缝险胜的问题随北移消失）。
...
流水线布局的作业对象是 17 个箱/包（弯道形态铺满；直线回退默认 5），**两种交错
排布**（默认 D01 + 顶部压皱的 D02）：

| key | 来源 | 尺寸（沿带 × 横向 × 高） | 质量 | 资产 |
|---|---|---|---|---|
| `d01` | v61 背景 `ConveyorBelt_Box_XX` 同款纸箱 | 0.25 × 0.38 × 0.1487 m | 1.0 kg | `props/cart_box_d01_physics.usda` |
| `d02` | Simple Warehouse 顶部压皱纸箱，任务层蓝色覆盖 + 白色顶标 | 0.25 × 0.38 × 0.1663 m | 1.0 kg | `props/cart_box_d02_physics.usda` |
...
默认排布是 `d01, d02` 循环交错（`ISAACLAB_BELT_BOX_PATTERN`，写单个 key 就是
全用一种；三种 `parcel_a0*` 软包裹仍可显式选用）。**默认（弯道形态）**箱子按**显式槽位序列**
排在工位 `s_stop=12.1945`（`y_stop=14.398`）上游，以队首 `s=8.06` 为锚、pitch 0.75
向上游**铺满整条路径**：`s = 8.06 − 0.75k (k=0..16)`——主线 1 箱（刚拐出弯
`y≈18.53`）+ 弧上 3 箱（"绕弯而来"的揭示点）+ X 支线 13 箱，最深槽位 `s=-3.94`
`(-16.70,20.05)` 藏在货架排 B 后面；`z=0.775`。最深槽位逐项核算：对支线滚筒可用端
`s_min=-4.27` 的净距按 d01/d02 路径半长 0.19 算 = **140 mm ≥ 50 mm**；比旧深藏位
`-3.90`（=回生点，**回生点不动**）更深 40 mm ⇒ E2"东上角"全遮（边界 `s≤-3.81`）
裕量 90→130 mm，遮挡结论只强不弱（E1 缝隙残余口径不变，见「已知限制」）。
...
弯道形态铺满后排队空间已贴着上限用：默认 17 箱等距 pitch 上限 ≈0.758（队尾缘顶
到支线可用端 `s_min=-4.27`）；显式减 COUNT 才能加大 pitch（如 `COUNT=5` 时最大约
3.0）。队首默认行程 `s 8.06→12.1945 ≈ 4.13 m`（约 17 s 流到工位）。直线回退形态维持
...
```text
settled_in_bin[i] = 箱底根在 blue_sorting_bin_01/_02 保守内区
                    & 线速度 <= 0.10 m/s & 角速度 <= 0.50 rad/s
drop_completed[i] = settled_in_bin 连续 10 个事件周期（约 0.2 s）后锁存
transfer_complete = all(on_belt[i] | drop_completed[i])
...
* 机器人把工位箱拎走 → 它既不在带上也不在目标框 → **整带继续停止**；
* 箱子在任一机器人身后的蓝色分拣框内部稳定约 0.2 s → 本次取放闭环完成并锁存 →
  整带重新启动，一起前进到新队首也压到工位为止。

于是每完成一次"取件→投框"，整列前进一格（≈ 出生间距）。搬运中或箱子掉到地上都不会
提前放行；深藏队尾也同格推进，要连续完成多次投放才逐渐拐出货架后面
——这正是"看不到头"的补货观感。
...
**净空隙**而不是中心距。当前 D01/D02 横向尺寸相同，但逐箱型半长口径仍支持显式切换到
不同尺寸的软包裹。

投放完成按“每个箱子 × 每个 env”锁存，F12/DDS/env reset 时清零。锁存避免已经完成的
箱子因框内堆叠、碰撞或后续清框而让流水线中途反悔；同时下一箱被抱起仍会形成新的
未完成缺口，所以首次投框不会永久解锁后续周期。几何、驻留和整带判据都在
...
箱子被举离带面、掉到地上或放到范围外都不会被强行拖动，也不会触发下一格放行。目标框
接收区保守沿用 0.25 m 内缩（覆盖当前最大箱型的 0.25 m 半边长），要求箱底根落到框底附近
并低速驻留，避免手持箱只从框沿上方经过或抛入弹跳时误触发。该投框门当前只用于默认停止式
`legacy` 后端；循环模式及 `surface_velocity` / 纯视觉输送机诊断组合维持原有行为。
...
"东上角"判据全遮、E1 有缝隙残余，见「已知限制」；比 17 箱的最深出生槽位 `-3.94`
浅 40 mm——回生点未随铺满改动）；直线回退形态维持主线 `y_respawn=18.25`。
**环路容量已复核并 fail-fast**：环路等价周长 = `s(y_recycle) − RESPAWN_S ≈ 19.64 m`，
17 箱队列跨度 12.0 m，回绕缺口 ≈7.64 m ≫ 首尾防撞下限（半长和 + queue_gap ≈0.45）
...
| `ISAACLAB_BELT_BOX_PATTERN` | `d01,d02` | 箱型循环序列（5 种可选）；未知 key 直接报错 |
...
验收（`--pick-lead-at` 会在指定步把队首搬离带面，保持 `--place-lead-after` 步确认
停线，再把它放入 robot_1 侧蓝框并检查下一格补位；弯道形态行程≈4.13 m ⇒ 默认
1600 步）：

```bash
python tools/smoke_conveyor_scene.py --steps 1600 --sync 0 --device cpu
python tools/smoke_conveyor_scene.py --steps 1600 --sync 0 --device cpu --pick-lead-at 1000 --place-lead-after 50
```
...
- **纸箱队列尚未做机器人实抓验收**：`--pick-lead-at` 是把队首直接搬离带面、等待后再
  写入蓝框来模拟完整取放，验的是"搬运中保持停线、框内稳定后正确放行"，不等于 Dex3
  真能抓起并投放箱子。当前 `d01`/`d02` 横向均为 0.38×0.25 m / 1.0 kg，
  已消除原 `c01` 的大跨度差异，但真实抓取仍需现场验收。
...
- `surface_velocity` 后端下纸箱队列靠"后车撞前车"物理涌现，没有走
  `queue_drive_mask`，也不支持本节的投框放行门；该组合尚未实测，上游带面会持续挤压
  排队的箱子。
- **回生点遮挡：E2 达成全遮，E1 几何上无全遮解（缝隙残余）**。Δ=0.25 北移 +
  五段支线把出生/回生点深藏到 `(-16.66,20.05)`（排 B 后面、近端还有叉车），
  离线 mesh 级射线核算（眼位 E1=(-4.54,14.698,1.6)/E2=(-6.7,14.698,1.6)，箱顶
  历史核算覆盖过更高的 c01；当前最高 d02=0.9413，E2 遮挡结论只会更强）：
  - **E2（robot_2 侧）**：按"东上角"判据**全遮**——顶面 25 点 + 四顶角 + 顶心全部
...
- **弧段拖滑比直线慢**（同款物理，弯道形态新增账目）：弧段实测拖滑 ~0.218 m/s
  ...
  （中心距下限 = 半长和 + queue_gap ≈ 0.49）会让后车临时停走——17 箱长队列在
  弧口会出现走走停停的"手风琴"瞬态，属预期。"停稳间距=出生间距"对拖滑物理
  不成立，smoke 判据用"队首停位 + 每对间距落在
  [max(0.45, 出生间距−0.45), 出生间距+0.35]"。
...
- 弯道压住背景红色地贴 `FloorZone_Robot`（随 Δ 同移后北缘 19.14）约 0.68 m：
  ...
  几何与 Δ=0 相同）。
- 弯道/支线的 GUI 目视验收（穿模/接缝/材质）尚未做——本次几何全部为离线 pxr
  实测 + smoke 动线验证；北移后的三条地贴（S3 远程资产）离线取不到包围盒，
  同样待 Kit 目视复核。
...
| props/cart_box_c01_physics.usda | 本仓库任务专用物理层 | 旧 C01 大箱封装；当前已从流水线箱型表移除，仅保留资产以便历史场景回溯 |
