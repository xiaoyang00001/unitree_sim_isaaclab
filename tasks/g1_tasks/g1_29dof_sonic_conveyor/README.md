...
| 值 | 布局 | 关键世界坐标 |
|---|---|---|
| `1`（默认） | 17 个箱/包沿**西拐入口弯道路径**以 pitch 0.75 排在工位上游；前两箱成组到位后供 robot_1/2 同时抱取，流水线持续停住且不再补位；生产默认前三个站位是 SONIC 动力学真身，robot_4/5 是纯显示 standby | 第一/第二机器人工位分别为 `y=13.998 / 14.748`（`x=-4.74 / -6.50`）；robot_3 优先接管靠近它们的原 StandbyRobot2，位于 `(-6.50,17.95)` 并朝 `+X`；robot_4 对应远端 `(-12.70,18.9534)` 并朝西北，robot_5 对应 `(-9.50,21.15)` 并朝 `-Y`；出生槽位 s=`11.06 − 0.75k (k=0..16)`；`y_stop=13.998` |
...
流水线有效带宽由 `0.90 m` 收窄为 `0.60 m`，主线车道中心仍为 `x=-5.62`；所有任务
箱/包的质心都放在路径中心线上，不再给前两箱添加朝机器人一侧的横向偏移。生产默认
随机池已移除横向尺寸为 `0.50 m`、在窄带上单侧仅余 `50 mm` 的 `c01/c02`；默认最宽的
...
约 `66.7 mm`。这两套桌箱与 robot_1/2 又作为两个刚性作业组统一沿世界 `-Y` 下移
`0.40 m`，机器人 Y 变为 `13.998/14.748`，收纳箱中心 Y 变为约
`15.148/15.898`；默认停止线同步下移到 `13.998`，所以前两箱仍分别正对两台机器人。
按 D02 的完整平面尺寸核算，从第二停位直线搬到西侧收纳箱的保守扫掠与北端高架
留下约 `38 mm` 正交净距；`0.30 m` 仍会相交约 `26 mm`，`0.35 m` 仅余约 `6 mm`。
robot_1 中立姿态与南端高架的二维 AABB 仍留约 `144 mm`。箱底与桌面贴合关系不变，
...
`y[10.44,18.47]`），车道中线 `x=-5.62`、有效带宽 `0.60 m`、带面 `z≈0.772`。
A08 只沿横向按 `2/3` 缩放，纵向长度和段间插接不变。**整条流水线（主线+弯道+
X 支线+机器人工位，=0 布局作业组同理）整体北移 Δ=0.25 m**：这是原 `0.90 m`
宽支线越过货架排 B 北侧时确定的基准（当时支线机身南缘 `19.4779` 对端护板北缘
`19.3946` 净距 `83.3 mm`，最小可行 Δ=0.2167 后取 50 mm 整倍数）。窄化后继续保留
这组纵向坐标，净距增至 `275.1 mm`，无需再次移动工位、路径和箱队；五段支线仍把
出生/回生点藏到货架（和叉车）后面。**货架与叉车都没有动**——动的只有流水线。
`conveyor_drive.CONVEYOR_NORTH_SHIFT_Y=0.25` 是唯一真源（`scene_layout` 抄一份、
测试交叉断言；`endless_intake` 用绝对坐标 +Δ 重钉、关系锁在测试层），几何落地在
clean wrapper 的 `over "ConveyorBelt"` 组变换 + 15 件带面装饰（世界 y +Δ ≡ 背景
局部 x +Δ）；地面标识已全部失活，不再需要随流水线位移。⚠️
`ISAACLAB_CONVEYOR_BACKGROUND=legacy_v61` 的裸 v61
...
**0.60 m 宽 A02 低位滚筒弯道向西（-X）拐 90°**，再接**五段同宽 A05 短直段**成
X 支线，箱子"从西侧
...
| 件 | 摆位 | 占位 |
|---|---|---|
| A02 窄弯道（`conveyor_belt_a02_narrow_visual.usdc`，scale=0.01） | pos `(-7.1129, 20.5548, 0.003)`，yaw `-90°` | `x[-7.113,-5.233] y[18.457,20.390]`，连续结构顶 `z=0.801`、门柱 `1.169` |
| A05 支线 ×5（`ConveyorBelt_A05_PR_NVD_01`） | t.x `-7.0979 / -9.0830 / -11.0681 / -13.0532 / -15.0383`（步进 1.9851），t.y `20.3877`、yaw `-90°`、scale `(0.006667,0.01,0.01)` | 合计 `x[-17.038,-7.098] y[19.670,20.437]`，滚筒可用带西端 `-17.034` |

插接：弯道**南向母口套主线带头公头 15 mm**（带头实测面 `y=18.4723`、公头中心
`x=-5.617`）；A02 围绕“主线半直线 + R=1.4 m 圆弧 + 支线半直线”的中心线逐顶点
把横向偏差缩为 `2/3`，因此中心线、转弯半径及两端接口纵向位置不动。弯道西公头
插段 1 母口、段间公插母各 15 mm。生成器与源资产指纹契约见
`tools/build_narrow_conveyor_curve.py` 及同名 manifest。周边间隙（mm，全部由离线 mesh
边界推导）：窄支线机身南缘
`19669.7` − 排 B 端护板北缘 `19394.6` = **275.1**，对架板/钢架柱净距分别约
323.7/362.4；对叉车货叉尖 `19321.0` 的净距为 **348.7**（段 5 与货叉 x 重叠段）；
Cone_4 南缘 − 支线北缘约 793.0，+Y 墙面 − 支线北缘约 3169.0（对窄弯道约
3216.2），柱 1473 落地柱身
南缘 − 支线北缘约 1559.0（该柱 y<20.9 部分全在 z≥7.96 的屋面桁架）；均无碰撞。

物理托面：默认 `legacy` 后端使用 `props/conveyor_path_support_physics.usda`
提供一张连续静态三角托面，一体覆盖主线 `y[10.44,18.47]`、拐角
`x[-7.02,-5.32] y[18.47,20.3534]` 和支线
`x[-17.034,-7.02] y=20.0534±0.30`；主线同样是 `x[-5.92,-5.32]`。顶面是单个连通
Mesh，没有旧主线/拐角/支线 Cuboid 对接处的
...
（s 参数化对 Δ 整体平移**不变**；本轮作业组另沿世界 `-Y` 下移 `0.40 m`，所以工位
s 相应增加 `0.40`）：工位 `y=13.998 ⇒ s=12.5945`、回收线
`10.85 ⇒ s=15.7425`、`s_min=-4.27`（支线滚筒
西端）。生成门 = `TOTES_ON_CONVEYOR=1` 且 `ISAACLAB_CONVEYOR_PROPS=layout`：
=0 拖车组（约 76%）与 legacy_props 空车车心都落在窄弯道占位内，被门拦下时
启动日志自曝原因并回退直线带头。

### 纸箱队列的整带节拍启停

流水线布局的作业对象是 17 个箱/包（直线回退默认 5），共注册 C01/C02、D01~D05
和三种软包裹；生产默认只混排 D01~D05 与三种软包裹，C01/C02 仅在显式 pattern 中使用：
...
沿带尺寸决定。默认队首 `d01` / `d02` 仍都是 0.25 m，类型不变，且与其余箱/包一样
统一居中在 `x=-5.62`。
...
随机；后 15 件从 `d01,d02,d03,d04,d05,parcel_a01,parcel_a02,parcel_a03`
八种窄带安全资产池中生成**带 seed 的平衡随机序列**。每轮先将八种各放一次再开始
...
```text
d01,d02,d05,parcel_a02,parcel_a01,d02,d03,d04,parcel_a03,d01,d05,d03,d04,parcel_a02,parcel_a03,parcel_a01,d01
```
...
排在工位 `s_stop=12.5945`（`y_stop=13.998`）上游，以队首 `s=11.06` 为锚、pitch 0.75
...
仍有 **3.14 m**；即使其他 seed 让默认池最大的 `parcel_a02` 落在队尾，也有不少于
**3.10 m**。循环模式的
...
直排，**默认 5 箱**（上游带面约 4.47 m，仍装不下 17），出生 y
`15.20 / 15.95 / 16.70 / 17.45 / 18.20`。

默认 `legacy` 的连续无缝托面已取代上述三块 Cuboid，因此随机序列中的软包裹和
D 箱可以位于支线或弧段，不再靠“只把 parcel 放在 3~5 号槽”规避内部竖直缝。
...
3.782，当前 seed 的第 5 件为 `parcel_a01`）。队首默认行程
`s 11.06→12.5945 ≈ 1.535 m`（按既有实测带速约 0.244 m/s 粗估约 6.3 s 流到
工位）。直线回退形态的上游带面约为 `18.47 - 13.998 ≈ 4.47 m`，
`(count-1)×pitch + 端部半长 + 队首行程 ≤ 4.47`，超了启动即报"队尾悬出带面"。
...
first_batch_arrived    |= any(arrived[0:2])
belt_running           = (新队首尚未到 y_stop) & ~first_batch_arrived
...
* 机器人只把工位箱竖直抬高 → 该箱不再受带速驱动，但 XY 仍在线上方 →
  **整带继续停止**；
* 两台机器人把首批双箱抬高或横向移出流水线通道 → 到位锁存保持，整带不会重新启动，
  第三箱不补位；只有 F12、DDS 或 env reset 清除锁存后，流水线才重新运行。
...
**净空隙**而不是中心距。默认八种小箱/包会混排；显式 pattern 仍可使用全部十种，
防撞判定始终按每件自己的 `max(沿带尺寸, 横向尺寸) / 2` 保守占位计算，不会把
C 型大箱当成 D01/D02。

到位状态按“每个箱子 × 每个 env”锁存，F12/DDS/env reset 时清零。前两箱作为同一
取件批次，任一到位就锁存两箱并形成终止停线；后续抱取、横移和回摆都不会解锁下一
周期。平面几何和整带判据都在
`conveyor_queue`（不依赖 Isaac，有单测）；状态只由物体权威端维护，viewer/镜像端继续
通过场景同步看到相同结果，无需新增 wire 字段。

驱动只在箱底位于带面高度 `z=0.772±0.15`，且原点进入主线矩形 `x=[-6.02,-5.22]`
`y=[10.44,18.47]` **或**弯道形态的两块附加矩形（拐角/支线，各放宽 0.10）时生效；
箱子被举离带面或掉到地上都不会被强行拖动。默认双机停止式场景不再使用“偏出通道即
放行”的节拍：首批到位后，无论箱根是否偏出矩形并集，流水线都持续停住。该门控只用于
停止式
...
17 箱队列跨度 12.0 m，回绕缺口 ≈7.64 m ≫ 首尾防撞下限（按显式 pattern 仍支持的
最大 C 型箱计也只有
...
| `ISAACLAB_BELT_BOX_PATTERN` | 未设（使用上述固定队首 + 八种小件的 seed 平衡随机） | 显式设置时完整覆盖默认序列并忽略 RANDOM_SEED，从第 1 件起按 pattern 循环；10 种 key 可选（含默认池已移除的 `c01/c02`），未知 key 直接报错 |
...
| `ISAACLAB_CONVEYOR_SPEED` / `_Y_STOP` / `_ENABLED` | `0.3` / `13.998` / `1` | 速度、工位、总开关；默认停止线跟随 robot_1 下移，弯道形态要求 y_stop 在主线段（<18.6534） |
...
`--depart-lead-after` 步确认停线，再横向移出通道并检查后续箱子仍不补位；旧参数名
`--place-lead-after` 仍兼容；弯道形态队首行程约 1.535 m，默认 1600 步还为
整列稳定和离线后持续停线留出时间）：
...
2026-08-11 历史组合实测：193 项相关单测通过；当时默认 seed 的 1600-step 停位与
...
- 停位有 0.06~10 mm 的散布（越线后驱动关闭、靠摩擦停住，各箱摩擦略有差异），smoke 容差
  ...
  出生 0.75），但**不要指望它精确恒定**。
- **直线回退形态**的队首行程随停止线下移增至 1.20 m（按 0.244 m/s 粗估约
  4.9 s）。带面上游总长约 4.47 m，间距、行程、数量三者仍需一起权衡。
- **纸箱队列尚未做机器人实抓验收**：`--pick-lead-at` 是把队首原地抬高、等待后再
  横向移出通道来模拟取放，验的是"首批到位后持续停线、XY 偏出后仍不补位"，不等于 Dex3
  真能抓起并投放箱子。本轮首批固定为 `d01`/`d02`（0.25×0.38 m / 0.1 kg），默认后续也
  只会出现八种窄带安全小件；`c01`/`c02` 已退出默认随机池。首批实抓仍需覆盖两件固定箱，
  显式启用 C 型大箱时则需另做可达性验收，不能把 headless 输送 smoke 视为 Dex3 抱取通过。
...
- **回生点遮挡：既有高度下 E2 达成全遮，C02 待复核；E1 几何上无全遮解（缝隙残余）**。Δ=0.25 北移 +
  ...
  下列离线 mesh 射线数字是窄化前眼位
  E1=`(-4.54,14.698,1.6)` / E2=`(-6.70,14.698,1.6)` 的历史核算；当前机器人内移后的
  眼位再随作业组下移，约为 E1=`(-4.74,14.298,1.6)` /
  E2=`(-6.50,14.298,1.6)`，需另行重跑，因此这些
  结论只作保守参考。历史核算最高只覆盖到 C01；显式 pattern 可启用的 C02 箱顶约 `z=1.086`，
  比 C01 再高 61 mm：
  - **历史 E2（robot_2 侧）**：既有 C01 及以下高度按"东上角"判据**全遮**——顶面 25 点 + 四顶角 + 顶心全部
    被排 B 首层架板板面实体挡住（遮挡源稳健）；全遮边界 `s≤-3.81`，`RESPAWN_S=-3.90`
    留 90 mm 裕量。残余=东立面中段 `z≈0.86-0.94` 有 4/20 点经架板下缝可见。
  - **历史 E1（robot_1 侧）无全遮解**：mesh 级复测推翻了"排 B 首层=实心遮挡体"的 AABB
...
- **弧段拖滑比直线慢**（同款物理，弯道形态新增账目）：弧段实测拖滑 ~0.218 m/s
  ...
  （中心距下限 = 该对半长和 + queue_gap；显式 pattern 的最坏 C/C 组合为 0.57 m）会让后车临时
  停走——17 箱长队列在
...
- 弯道/支线的 GUI 目视验收（穿模/连续托面/材质）尚未做——本次几何全部为
  离线 pxr 实测 + smoke 动线验证，仍待 Kit 目视复核。
...
| warehouse-simple6_v61_visual_only.usda | 本仓库任务专用强覆盖层 | 默认背景入口；引用原始 v61，对 10 个纸箱和 5 个 KLT 料箱删除刚体/碰撞 API 并显式禁用物理，**再用 `active=false` 整体摘出组合**（摆位与带面对不上，且与任务的真箱子队列重叠）；同时移除工位装饰箱堆、交通锥和全部地面标识，并作为**整体北移 Δ=0.25 的几何落地处**（`over "ConveyorBelt"` 组变换 + 15 件失活带面装饰，真源 `conveyor_drive.CONVEYOR_NORTH_SHIFT_Y`） |
