...
| 值 | 布局 | 关键世界坐标 |
|---|---|---|
| `1`（默认） | 5 个纸箱沿**西拐入口弯道路径**排在工位上游、挡停放行流向工位；机器人分站流水线两侧 | 工位 `y=14.148`（机器人对称分站 `x=-4.54 / -6.7`，各距带中线 1.08 m）；队首出生 s=8.06（主线 `y≈18.28` 刚拐出弯，间距 0.75 沿路径向上游排，队尾在 X 支线 `(-7.70,19.80)`）；`y_stop=14.148` |
...
**A02 低位滚筒弯道向西（-X）拐 90°**，再接**三段 A05 短直段**成 X 支线，箱子"从西侧
货架方向拐出来"。**零新增货架/遮挡件**（用户硬要求）——遮挡只依赖背景 v61 既有的
西侧满载货架排（遮挡缺口见「已知限制」）。几何（世界系，全部离线 pxr 实测）：

| 件 | 摆位 | 占位 |
|---|---|---|
| A02 弯道（`ConveyorBelt_A02_PR_NVD_01`，scale=0.01） | pos `(-7.1129, 20.3048, 0.003)`，yaw `-90°` | `x[-7.113,-5.042] y[18.207,20.308]`，连续结构顶 `z=0.801`、门柱 `1.169` |
| A05 支线 ×3（`ConveyorBelt_A05_PR_NVD_01`） | t.x `-7.0979 / -9.0830 / -11.0681`，同 yaw/z | 合计 `x[-13.068,-7.098] y[19.228,20.379]`，滚筒可用带西端 `-13.064` |

插接：弯道**南向母口套主线带头公头 15 mm**（带头实测面 `y=18.2223`、公头中心
`x=-5.617`）；弯道西公头插段 1 母口、段间公插母各 15 mm。三段是**硬上限**：端头
`-13.068` 距背景货架排 B 端护板东缘 `-13.386` 仅余 `0.318 m`，第四段必与货架排
（`y≤19.346`）干涉。弯道北缘距 +Y 墙 `3.30 m`；交通锥 Cone_4 距支线 y 净距
`0.85 m`；均无碰撞。

物理托面：两块 kinematic 补板与主线碰撞板同 z 同摩擦——拐角补块
`x[-7.02,-5.17] y[18.22,20.2534]` + 支线条带 `x[-13.064,-7.02] y=19.8034±0.45`
（顺带补齐段间 41 mm 滚筒断口；条带南缘与货架排北缘 7 mm 错缝但 x 完全分离）。
A02/A05 本体纯视觉（coll=0/rigid=0），全部 `AssetBaseCfg` → `scene.extras`，
不进 scene_props/同步清单。

驱动换**沿路径距离 s** 的两段式（判据在 `conveyor_queue`，零 Isaac 依赖有单测）：
支线 +X → R=1.4 圆角弧（圆心 `(-7.02,18.4034)`，由两条车道中线导出）→ 主线 -Y；
...
工位 `y=14.148 ⇒ s=12.1945`、回收线 `10.6 ⇒ s=15.7425`、`s_min=-0.30`（支线滚筒
...
写单个 key 就是全用一种；`d01,c01` 切回双纸箱形态）。**默认（弯道形态）**箱子沿入口
弯道路径排在工位 `s_stop=12.1945`（`y_stop=14.148`）上游，出生 s 为
`8.06 / 7.31 / 6.56 / 5.81 / 5.06`（沿路径间距 0.75）——主线 1 箱 + 弧上 3 箱 +
支线 1 箱，开局即"绕弯而来"；`z=0.775`。`belt_box_1` 是队首。
`ISAACLAB_CONVEYOR_ENDLESS=off` / `legacy_props` 回退直线形态：沿主车道 `x=-5.62`
直排，出生 y `14.95 / 15.70 / 16.45 / 17.20 / 17.95`。

弯道形态下排队空间富余（支线多给 ~4.4 m）：pitch 最大约 2.0（默认 s_lead 下），
队首默认行程 `s 8.06→12.1945 ≈ 4.13 m`（约 17 s 流到工位）。直线回退形态维持旧账：
上游只有 `18.22 - 14.148 ≈ 4.07 m`，`(count-1)×pitch + 端部半长 + 队首行程 ≤ 4.07`，
超了启动即报"队尾悬出带面"。
...
`y=[10.19,18.22]` **或**弯道形态的两块附加矩形（拐角/支线，各放宽 0.10）时生效；
箱子被举离带面、掉到地上或放到范围外都不会被强行拖动。
驱动只在物体权威端运行，viewer/镜像端通过场景同步看到相同运动。

`ISAACLAB_CONVEYOR_Y_STOP<=0` 切到**循环模式**：没有工位停止线，整带长跑不停（只剩防撞
保底），箱子到 `y_recycle=10.6` 后被传回回生点继续循环（回收由额外挂上的
`recycle_surface_totes` 事件负责——纸箱的驱动函数只管启停和限速，不搬运）。弯道形态
的回生点在 X 支线最深处 `(-12.61, 19.8034)`（距双机眼位 8.0~9.6 m，pop-in 几何上
仍可见，见「已知限制」）；直线回退形态维持主线 `y_respawn=18.0`。
...
| `ISAACLAB_BELT_BOX_SPAWN_S_LEAD` | `8.06` | 弯道形态队首出生的沿路径距离（优先于 Y_LEAD） |
| `ISAACLAB_BELT_BOX_SPAWN_Y_LEAD` | `14.95`（直线） | 队首出生 y（世界系），同时是复位落点；弯道形态只接受主线段 y（<18.4034）并自动换算成 s |
| `ISAACLAB_BELT_BOX_SPAWN_PITCH` | `0.75` | 箱间距（弯道形态=沿路径距离）；整带节拍下**也就是**停稳后的队列间距 |
...
| `ISAACLAB_CONVEYOR_SPEED` / `_Y_STOP` / `_ENABLED` | `0.3` / `14.148` / `1` | 速度、工位、总开关；弯道形态要求 y_stop 在主线段（<18.4034） |

下列非法组合都会在启动时报错，不会等到运行时才看见箱子悬空或互相穿模：出生位排到
带面/支线外、**相邻两箱按各自半长算放不下**、队首压在工位上、工位不在主线段、
Y_LEAD 排到弧段/支线（须换 S_LEAD）、箱型比带面还宽、未知箱型 key。

验收（`--pick-lead-at` 会在指定步把队首搬离带面，模拟机器人取件；弯道形态行程
≈4.13 m ⇒ 默认 1600 步）：

```bash
python tools/smoke_conveyor_scene.py --steps 1600 --sync 0 --device cpu
python tools/smoke_conveyor_scene.py --steps 1600 --sync 0 --device cpu --pick-lead-at 1000
```
...
- **"零新增遮挡即全遮"在几何上无解（需用户拍板）**：离线射线核算（眼位
  E1=(-4.54,14.448,1.6)/E2=(-6.7,14.448,1.6)，箱顶最高 c01=1.0223）——已有货架
  排 B 的全遮阴影区要求箱心 `x≤-14.36`（E1）/`-14.22`（E2），支线物理极限只到
  `-13.07`（排 B 端护板 `-13.386` 硬限），差 1.15~1.36 m 不可达；其余候选遮挡
  全部证伪（3M44 是 22.6 m 外的西边界墙、InnerCorner 在西南角、"立柱"落地段
  不存在、带体挡边顶 0.801 低于箱顶）。逐 slot：主线/弧上箱子裸露~半露（箱底沿
  被挡边遮），支线段箱顶可见、下半 `z<0.80` 被支线自身挡边遮；循环模式回生点
  `(-12.61,19.80)` 的 pop-in 几何上可见（距眼 8.0~9.6 m，背景是排 B 满货+叉车，
  观感弱化）。可选路径（均未实施）：①只挪现有道具——把背景叉车
  （`x[-16.09,-14.87]`）东移 ~1.9 m 停到支线端头正南可罩住端头段+回生点，
  最贴近"不新增货架"的精神；②端头加非货架遮挡件（违背字面要求）；③接受现状。
- **弧段拖滑比直线慢**（同款物理，弯道形态新增账目）：弧段实测拖滑 ~0.218 m/s
  低于直线 ~0.244（东拐版实测，同族资产/同套托面适用），过弯时队距被拉伸至
  ~0.80-0.86（出生 0.75）——"停稳间距=出生间距"对拖滑物理不成立，smoke 判据
  用"队首停位 + 间距区间 [0.45, pitch+0.25]"。
- `surface_velocity` 后端只驱动主线碰撞面（-Y）：支线/弧段上的箱子不会被接触
  驱动，该组合未验证（启动日志有警告），弯道形态建议 legacy 后端。
- 弯道压住背景红色地贴 `FloorZone_Robot`（北缘 18.89）约 0.68 m：纯视觉贴纸
  z=0.01 无碰撞，观感是"设备摆进了机器人红区"，暂接受未收缩。
