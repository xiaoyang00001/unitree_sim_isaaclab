...
3.782，当前 seed 的第 5 件为 `parcel_a01`）。队首默认行程 `s 11.06→12.1945 ≈ 1.135 m`（实测带速约 0.244 m/s 时约
4.6 s 流到工位）。直线回退形态维持
...
- **回生点遮挡：既有高度下 E2 达成全遮，C02 待复核；E1 几何上无全遮解（缝隙残余）**。Δ=0.25 北移 +
  ...
  眼位约为 E1=`(-4.74,14.698,1.6)` / E2=`(-6.50,14.698,1.6)`，需另行重跑，因此这些
