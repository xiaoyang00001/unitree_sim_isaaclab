...
开局相机站在流水线开头（入料端 `y=18.222`）正对着流水线看下去：`eye`/`lookat` 的
`x` 都压在带面中线 `-5.62` 上，视线方向是纯 `-Y`，与筐的流动方向一致。

**机位跟着布局走**，因为两种布局的作业组差 4.6 m：

| 布局 | eye | lookat | 距作业组 |
|---|---|---|---|
| `1` | `(-5.62, 18.80, 2.8)` | `(-5.62, 14.148, 1.0)` | 4.65 m |
| `0` | `(-5.62, 22.75, 2.8)` | `(-5.62, 18.750, 1.0)` | 4.00 m |

取值是 `max(ROBOT_WORKSTATION_Y + 4.0, 18.8)`：既要退到作业组后方，又要至少站到
入料端外。下限 `18.8` 而非更靠后，是因为 `=1` 布局的空拖车 `pushcart_2` 停在
`y=19.394`，18.8 正好夹在入料端与拖车之间各留约 0.6 m。

⚠️ **别把 eye 写死**。写死成适配某一种布局的值，另一种布局下就会贴脸——例如按 `=1`
推出的 `(-5.62, 19.0, 2.4)`，在 `=0` 下距作业组只剩 0.25 m 且正对拖车中心。
GUI 里飞相机只是临时的，要把飞好的位置固化回代码，用 `--full_kit` 开 Script Editor
读 `vp.camera_path` 的 world transform。
...
- ⚠️ **切到 v61 后，默认布局（`=1`）的两台机器人站位落进了有碰撞的分拣料箱里。**
  v61 把 packing table 与 `blue_sorting_bin_01/02` 整体往 `-Y` 挪了约 0.5～0.6 m，
  而 `ROBOT_WORKSTATION_Y=14.148` 没跟着动：

  | | v48 料箱 y | v61 料箱 y | 机器人 (x, y) | 重叠 |
  |---|---|---|---|---|
  | robot_1 | `[14.60,16.00]` | `[14.13,15.36]` | `(-4.75, 14.148)` | 进 bin_02 约 1.8 cm |
  | robot_2 | `[14.60,16.00]` | `[13.99,15.32]` | `(-6.70, 14.148)` | 进 bin_01 约 16 cm |

  两个 `blue_sorting_bin` 都**有碰撞**且 z 跨 `[0.50,0.95]`（机器人腰部高度）；打包桌
  无碰撞，只是视觉穿模。`=0` 布局（机器人在 `y=18.75`）不受影响。判据是离线 USD
  包围盒比对，**未实跑确认弹飞/倒地复位的实际表现**。修法二选一：把
  `ROBOT_WORKSTATION_Y` 跟着桌子往 `-Y` 挪，或把 v61 里的桌子/料箱挪回 v48 位置。
  见[优化路线](../../../doc/conveyor_scene_optimization_roadmap_zh.md) §2.2。
