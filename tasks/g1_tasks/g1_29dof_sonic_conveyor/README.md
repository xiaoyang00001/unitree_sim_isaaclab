...
沿带尺寸决定，默认 `d01` / `c01` 分别为 0.25 / 0.50 m。
...
默认排布是 `d01, c01` 循环交错（`ISAACLAB_BELT_BOX_PATTERN`，写单个 key 就是
...
`s_min=-4.27` 的净距按最坏 c01 半长 0.25 算 = **80 mm ≥ 50 mm**；比旧深藏位
...
| `ISAACLAB_BELT_BOX_PATTERN` | `d01,c01` | 箱型循环序列（5 种可选）；未知 key 直接报错 |
...
- **纸箱队列尚未做机器人实抓验收**：`--pick-lead-at` 是把队首直接搬离带面、等待后再
  ...
  真能抓起并投放箱子。尤其 `c01` 是 0.5×0.5×0.25 m / 1.5 kg，跨度和重量都明显
  大于 `d01`，真实抓取仍需现场验收。
