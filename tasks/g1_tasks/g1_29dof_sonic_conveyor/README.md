...
| 值 | 布局 | 关键世界坐标 |
|---|---|---|
| `1`（默认） | 5 个纸箱沿**西拐入口弯道路径**排在工位上游、挡停放行流向工位；机器人分站流水线两侧 | 工位 `y=14.398`（机器人对称分站 `x=-4.54 / -6.7`，各距带中线 1.08 m）；出生槽位 s=`8.06/7.31/6.56/5.81/-3.90`（队首主线 `y≈18.53` 刚拐出弯 + 弧上 3 箱，**队尾深藏**在 X 支线最深处 `(-16.66,20.05)`=回生点、藏在货架排 B 后面）；`y_stop=14.398` |
...
写单个 key 就是全用一种；`d01,c01` 切回双纸箱形态）。**默认（弯道形态）**箱子按
**显式槽位序列**排在工位 `s_stop=12.1945`（`y_stop=14.398`）上游，出生 s 为
`8.06 / 7.31 / 6.56 / 5.81 / -3.90`——前四位维持间距 0.75（主线 1 箱 + 弧上 3 箱，
开局即"绕弯而来"的刻意揭示点），**队尾单列深藏**在支线最深处（=回生点
`(-16.66,20.05)`，藏在货架排 B 后面，等前面被逐一取走才逐格拐出来）；`z=0.775`。
`belt_box_1` 是队首。显式给出 `S_LEAD`/`Y_LEAD`/`PITCH` 任一旋钮时回退等距排布
（不深藏队尾）；`COUNT<5` 取序列前缀。
`ISAACLAB_CONVEYOR_ENDLESS=off` / `legacy_props` 回退直线形态：沿主车道 `x=-5.62`
直排，出生 y `15.20 / 15.95 / 16.70 / 17.45 / 18.20`。

弯道形态下排队空间富余（五段支线给 ~8.3 m）：等距 pitch 最大约 3.0（默认 s_lead
下），队首默认行程 `s 8.06→12.1945 ≈ 4.13 m`（约 17 s 流到工位）。直线回退形态维持
...
后续箱子各前进一格后同时停住；深藏队尾也同格推进，要连取多件才逐渐拐出货架后面
——这正是"看不到头"的补货观感。
...
的回生点在 X 支线最深处 `(-16.66, 20.0534)`（=默认队尾深藏槽位，藏在货架排 B 后面：
E2 眼位按"东上角"判据全遮、E1 有缝隙残余，见「已知限制」）；直线回退形态维持主线
`y_respawn=18.25`。
...
| `ISAACLAB_BELT_BOX_SPAWN_S_LEAD` | 无（默认走槽位序列） | 弯道形态队首出生的沿路径距离（优先于 Y_LEAD）；**显式给出 S_LEAD/Y_LEAD/PITCH 任一都会回退等距排布**（不深藏队尾） |
...
- **弧段拖滑比直线慢**（同款物理，弯道形态新增账目）：弧段实测拖滑 ~0.218 m/s
  ...
  最多被拉伸 ≈ 弧长 2.2 × (0.244/0.218−1) ≈ 0.26（本轮 smoke 实测 1.011，
  出生 0.75）；深藏队尾在前车过弧期间自己还在直线段，间距反向收缩 ~0.3——
  "停稳间距=出生间距"对拖滑物理不成立，smoke 判据用"队首停位 + 每对间距落在
